...
public class GroundDropdown extends SubsystemBase {
  ...
  public void setDesiredSetpoint(Angle angle) {
    m_desiredAngle =
        Degrees.of(
            MathUtil.clamp(
                angle.in(Degrees), DROPDOWN.minAngle.in(Degrees), DROPDOWN.maxAngle.in(Degrees)));
  }
  ...
  private void updateLogger() {
    ...
    SmartDashboard.putNumber(
        "GroundDropdown/CanCoderAbsolutePos360", getCANcoderAngle().in(Degrees));
  }
  ...
  public void testInit() {
    ...
    m_kCruiseVel_subscriber =
        m_dropdownTab.getDoubleTopic("kCruiseVel").subscribe(DROPDOWN.kCruiseVel);
  ...
  public void simulationPeriodic() {
    ...
    m_dropdownEncoderSimState.setRawPosition(
        Units.radiansToRotations(m_dropdownSim.getAngleRads()));
    m_dropdownEncoderSimState.setVelocity(
        Units.radiansToRotations(m_dropdownSim.getVelocityRadPerSec()));

    SmartDashboard.putNumber(
        "GroundDropdown/Model Angle", Units.radiansToDegrees(m_dropdownSim.getAngleRads()));
  }
}
