...
public class GroundPivotJoystick extends Command {
  ...
  public void execute() {
    ...
    //   if (m_groundPivot.getControlMode() == ROBOT.CONTROL_MODE.CLOSED_LOOP) {
    //     var rotationSetpoint =
    //         MathUtil.clamp(
    //             m_joystickDeadband * 0.5 + m_groundPivot.getCurrentRotation(),
    //             Units.degreesToRotations(PIVOT.minAngleDegrees),
    //             Units.degreesToRotations(PIVOT.maxAngleDegrees));
    //     m_groundPivot.setDesiredSetpointRotations(rotationSetpoint);
    //   } else if (m_groundPivot.getControlMode() == ROBOT.CONTROL_MODE.OPEN_LOOP) {
    //     if (PIVOT.limitOpenLoop) {
    //       // Upper limit
    //       if (m_groundPivot.getCurrentAngle() >= PIVOT.maxAngleDegrees - 1)
    //         m_joystickDeadband = Math.min(m_joystickDeadband, 0);

    //       // Lower limit
    //       if (m_groundPivot.getCurrentAngle() <= PIVOT.minAngleDegrees + 1)
    ...
    //   }
    // } else {
    //   if (m_groundPivot.getControlMode() == ROBOT.CONTROL_MODE.OPEN_LOOP)
        m_groundPivot.setPercentOutput(m_joystickDeadband * PIVOT.joystickMultiplier);
    // }
  }
